...
/**
 * The two formations, and how each one carries itself. Offsets are from the
 * leader; the sizes fake the depth of a loose echelon.
 */
const FORMATIONS = {
  flock: {
    kind: 'turtle',
    ...
    size: 0.055,
    gait: 'swim',
    skin: { texScale: 1.7, halfHeight: 0.17, plateMix: 0.8 },
    members: [
      { dx: 0, dy: 0, size: 1 },
  ...
  sky: {
    ...
    members: [
      { dx: 0, dy: 0, size: 1 },
      { dx: -0.46, dy: 0.13, size: 0.74 },
      { dx: -0.4, dy: -0.16, size: 0.64 },
    ],
  },
} as const
...
/**
 * One creature of a formation. Each keeps its own clock, which stays in step
 * with the others because they share one cached mesh and so all mount on the
 * same frame.
 */
function Member({ index, plan, theme, tier, bump, onDone }: MemberProps) {
  const spec = FORMATIONS[plan.kind === 'sky' ? 'sky' : 'flock']
  const rig = useCreature(spec.kind, DETAIL_BY_TIER[tier])
  const group = useRef<Group>(null)
  const clock = useRef(0)
  const spot = spec.members[index] ?? spec.members[0]!
  ...
  useFrame((state: RootState, dt) => {
    ...
    let lift = 0
    if (spec.gait === 'flap') {
      // Long glides broken by bursts of flapping, which is how anything with that
      // much wing actually travels. Offset per bird so they never beat in unison.
      const power = 0.32 + 0.5 * (0.5 + 0.5 * Math.sin(t * 0.55 + index * 1.4))
      lift = animateFlap(rig.byName, t + index * 0.45, 1.35, power)
    } else {
      animateTurtleSwim(rig.byName, t, index * 1.9)
    }

    const x = (MathUtils.lerp(-OFF, OFF, u) + spot.dx) * way
    const y = lane + spot.dy + Math.sin(t * 1.15 + index) * 0.035 + lift
    g.position.set((x * width) / 2, (y * height) / 2, 0)
    g.scale.setScalar(screen * spec.size * spot.size)

    const yaw = way > 0 ? -0.45 : Math.PI + 0.45
    if (spec.gait === 'flap') {
      // Banked over, because a wing seen edge-on is a line. `rotation.x` is the
      // outermost of an XYZ euler, so it rolls about the line of travel.
      g.rotation.set(0.44 + Math.sin(t * 0.5 + index) * 0.18, yaw, Math.sin(t * 0.8 + index) * 0.07)
    } else {
      g.rotation.set(Math.sin(t * 0.7 + index) * 0.12, yaw, Math.sin(t * 0.9 + index) * 0.09)
    }
  })
  ...
  return (
    <Body
      ...
      theme={theme}
      bump={bump}
      texScale={spec.skin.texScale}
      halfHeight={spec.skin.halfHeight}
      plateMix={spec.skin.plateMix}
      bodyRef={group}
    />
  )
}

/** Turtles gliding past, or pterosaurs crossing the sky above the reading. */
function Formation(props: RunProps) {
  const spec = FORMATIONS[props.plan.kind === 'sky' ? 'sky' : 'flock']
  return (
    <>
      {spec.members.map((_, i) => (
        <Member key={i} index={i} {...props} onDone={i === 0 ? props.onDone : undefined} />
      ))}
    </>
  )
}
...
export function Cameo({ plan, theme, tier, onDone }: CameoProps) {
  ...
  return plan.kind === 'chase' ? <Chase {...props} /> : <Formation {...props} />
}
